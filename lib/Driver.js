...
module.exports = class Driver {
    ...
    /**
     * Calculates and sets the average speed property.
     */
    setAverageSpeed() {
        const addDuration = (accumulator, trip) => accumulator + trip.duration;
        const addMiles = (accumulator, trip) => accumulator + trip.miles;
        const minutesDriven = this.trips.reduce(addDuration, 0);
        const hoursDriven = minutesDriven / 60;
        const milesDriven = this.trips.reduce(addMiles, 0);

        this.averageSpeed = milesDriven / hoursDriven;
    }
